...
def main():
    ...
    for query in desired_queries:
        try:
            ...
            for i in range(8):
                ...
                soup = create_soup_from_url(url='https://www.newsit.gr/page/' + str(page) + '/?s=' + quote(query))
                
                # Extracting number of link_results
                search = soup.find_all('main', attrs={'class':'site-main'})
                
                # Search for articles within given tag:
                for s in  search:
                    articles = soup.find_all('h2', attrs={'class': 'mb-1'})
                    
                    # Extract the link of each article:
                    for a in articles:
                        links = a.contents[0].get('href')
                        # print(links)  
    ...
    for i in range(len(link_results)):
        ...
        try:
            article_body = soup.find('div', attrs={'class':'entry-content post-with-no-excerpt'}).findAll('p', recursive=False)
        except:
            continue
        ...
        # Get article title
        try:
            article_title = soup.find('div', attrs={'class':'inside-article'}).find('h1', recursive=False)
    ...
    articles_df.to_csv(r'data/newsit_articles.txt', index=False, sep=' ', header=False)
    articles_df = articles_df['Article']
    articles_df.to_csv(r'data/plain_newsit_articles.txt', index=False, sep=' ', header=False)
